...
const UpdatePropertyPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || title.trim() === "") {
      toast.error("Please enter a valid property title!");
      return;
    }

    if (!description || description.trim() === "") {
      toast.error("Please enter a valid property description!");
      return;
    }

    if (!area || area.trim() === "") {
      toast.error("Please enter a valid property area! e.g.(4 Marla)!");
      return;
    }
    ...
    if (bedrooms <= 0 || isNaN(bedrooms)) {
      toast.error("Please enter a valid number of bedrooms!");
      return;
    }

    if (bathrooms <= 0 || isNaN(bathrooms)) {
      toast.error("Please enter a valid number of bathrooms!");
      return;
    }

    if (kitchens <= 0 || isNaN(kitchens)) {
      toast.error("Please enter a valid number of kitchens!");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/property/update/${propertyId}`,
        {
          method: "POST",
          headers: {
          ...
          <div className="flex flex-col flex-1 gap-6">
            <div className="flex flex-col gap-4">
              ...
              <input
                ...
                accept=".jpg,.png,.jpeg"
                multiple
                className="border-2 border-dashed p-4 rounded-lg text-gray-700 focus:border-green-600"
              />
              <button
                type="button"

      ...
      <div className="p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {serviceOptions.map((item) => {
            ...
            return (
              <div
                className="bg-white shadow-xl p-6 text-center border border-gray-200 hover:border-green-700 rounded-lg flex flex-col"
                key={item.id}
              >
                <div className="flex justify-center items-center bg-green-200 rounded-full w-16 h-16 mx-auto shadow-md mb-3">
                  <IconComponent className="text-green-700 text-3xl" />
                </div>
                <div>
                  <h3 className="truncate text-lg font-semibold text-gray-800 hover:text-green-700 transition-colors cursor-pointer">
                    {item.title}
                  </h3>
